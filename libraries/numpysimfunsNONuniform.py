...
import logging as l
import pandas as pd
l.debug("NumpySimfuns_nonUniform with debug-messages")
def update(current, announcement,dist_df, simulations, agents, rel, favorOneself = False):
    ...
    # 5) Assign sor-randomly chosen opinions to the agents
    l.debug("current :\n"+ str(current) +"\n")
    l.debug("Announ :\n"+ str(announcement)+"\n")
    l.debug("num of optimal opins by agent :\n"+ str((optOpins.sum(axis=0))))

    opinstochoosefrom = optOpins[:, optOpins.sum(axis=0) >1]
    for agent in range(opinstochoosefrom.shape[1]):
        l.debug(f"looking at agent(col) {agent} with bool-col from {opinstochoosefrom[:,agent]}")
        indicestochoosefrom = np.arange(numOpins)[opinstochoosefrom[:,agent]]
        assert len(indicestochoosefrom.shape) == 1, "indicestochoosefrom was not an r1a, but rather \n{indicestochoosefrom}" 
        l.debug(f"  ->indicestochoosefrom is {indicestochoosefrom}")
        currentChoice = indicestochoosefrom[0]
        for i in range(len(indicestochoosefrom)-1):
            zoro = np.random.randint(2)
            if zoro == 1:
                currentChoice = indicestochoosefrom[i+zoro]
            l.debug(f"   -she chooses {currentChoice} (zoro = {zoro})")
        opinstochoosefrom[:,agent] = np.arange(numOpins) == currentChoice
        l.debug(f"thus her op2chfrom is: {opinstochoosefrom[:,agent]}")
    optOpins[:, optOpins.sum(axis=0) >1] = opinstochoosefrom
    l.debug(f"   =>optOpins is \n{optOpins} ")
    Choice = optOpins * helper # Multiplies a bool-array that is True at the unique (but possibly "random") choice with the array holding al opinion-indices
...
def deliberate(dist_df, rounds, agents, reliability, simulations, favorOneself = False, dynamicSimEnd = True):
    ...
    """
    import time as t
    # Agents should enter deliberation with strict rankings only.
    ...
    for rnum in range(rounds): 
        ...
        for step in range(agents): # each round is made of agents [int] steps, because everyone gets to speak their mind once (and only once).
            if step==0==rnum: updateStart = t.time()    
            ...
            if step==0==rnum: print("First Update took", round(t.time()-updateStart), "seconds")
