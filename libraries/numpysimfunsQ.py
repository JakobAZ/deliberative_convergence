...
import logging as l
import pandas as pd
l.debug("NumpysimfunsQ with debug-messages")
def update(current, announcement,dist_df, simulations, agents, rel, favorOneself = False):
    ...
    helper3d = np.repeat(np.repeat(np.arange(numOpins).reshape(-1,1,1), current.shape[0], axis = 1 ), current.shape[1], axis = 2  ) 
    # 2) get distances of all potential opinions from announcement and current opinion
    # 2.1 dFromAnnoun
    dFromAnnoun = np.array(dist_df.iloc[announcement].values.T.reshape(-1,simulations,1)).astype(np.float)
    # 2.2 dFromOp
    f = lambda n,dist_df,current,simulations: np.array(dist_df.iloc[current[:,n]].values.T.reshape(-1,simulations,1))
    ...
    if favorOneself: # Deducts from own opinion a minusule but tie-breaking amount if we want to favour ourselves.
        score[helper3d==current] = (score[helper3d==current]-(1E-5)) #This amount should be sufficiently small that the own opinion doesn't get preferred if it is not among the truly optimal.
    optOpins = score == np.min(score, axis = 0) # Gives a bool-array True whenever an opinion is optimal.
    l.debug(f"OptOpins (shape {optOpins.shape}): {optOpins}")
    # Maybe, multiple opinions are optimal. Then we need to decide on one. To save computing power, we do (somewhat complicated) slicing and reshaping and only draw random numbers for the agents that have no unique best choice!
    OO = optOpins.reshape(optOpins.shape[0], -1).T
    nonuniques = OO[(optOpins.sum(0) > 1).reshape(-1)]
    l.debug(f"nonuniques (shape {nonuniques.shape}): \n{nonuniques}")
    # 5) Assign randomly uniform values to the non-unique optimal opinions, if they exist.
    if nonuniques.shape[0] >0:
        l.debug(f"rows should hold the heights now: \n{OO}")
        ChoiceScore = np.random.uniform(0,1, (nonuniques.shape))
        uOO = ChoiceScore == (ChoiceScore*nonuniques).max(1).reshape(-1,1)
        OO[(optOpins.sum(0) > 1).reshape(-1)] = uOO # Attention, this assignment CHANGES optOpins!!!
    l.debug(f"optOpins now has shape {optOpins.shape} (as before), but for every agent per simulation holds a unique 'True' (This statement is {(optOpins.sum(0)==1).all().all()})")
    Choice = optOpins * helper3d # Multiplies a bool-array that is True at the unique (but possibly random) choice with the array holding al opinion-indices
...
def deliberate(dist_df, rounds, agents, reliability, simulations, favorOneself = False, dynamicSimEnd=True):
    ...
    for rnum in range(rounds): 
        ...
        for step in range(agents): # each round is made of agents [int] steps, because everyone gets to speak their mind once (and only once).
            if (step%5)==(rnum%5): updateStart = t.time()    
            ...
            if (step%5)==0==(rnum%5): print(f"{step}th Update took", round(t.time()-updateStart, 2), "seconds")
